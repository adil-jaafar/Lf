...
class Qs extends Service {
	...
	/*
	public function execQuery($q, $conditions = array() , $mainModel = false , $typeSortie = 'FETCH_ASSOC') {
		...
		if( !is_array( $sql ) ) {
			$sql = array( 'sql'=> $sql , 'conditions'=>array() , 'prequery'=>array() );
		}
		...
		if( isset( $sql['sql'] ) ) {
			...
			$sql[] = array( 'sql'=>$sql['sql'] , 'conditions'=>$sql['conditions'] );
			unset( $sql['sql'] );
			unset( $sql['conditions'] );
		}
		//var_dump( $sql );
		return false;
		//$sql = $sql['sql'];
		//$conditions = array_merge( (array)$conditions , (array)$sql['conditions'] );

		try {
			$db = $this->getConnexion();
			foreach ($sql as $key => $value) {
				$req = $db->prepare($value['sql']);
				$req->execute( $value['conditions'] );
				switch($typeSortie) {
					case 'FETCH_OBJ':
